...
| J4   | -180°  | 180°   | 腕部旋转                       |
| J5   | -120°  | 120°   | 腕部俯仰                       |
| J6   | -720°  | 720°   | 末端旋转（支持多圈）           |

运行 `python3 scan_multi_joint.py --show-limits` 可随时查看。
...
只动一个关节，其余固定。不指定角度时使用该关节的默认扫描范围：

```bash
# J3 默认 155→135°，只需指定 joint
python3 scan_multi_joint.py --mode arc --joint 3

# 自定义范围（超出硬件限制会打印 WARN）
python3 scan_multi_joint.py --mode arc --joint 3 \
...
| 参数            | 含义                                   |
| --------------- | -------------------------------------- |
| `--joint 3`     | 选关节 1-6，每个关节有各自默认扫描范围 |
| `--start-angle` | 起始角度，不填用该关节默认值           |
| `--end-angle`   | 终止角度，不填用该关节默认值           |
| `--steps`       | 分多少步                               |
